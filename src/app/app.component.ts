...
export class AppComponent {
	...
	constructor(router: Router, private storeService: StoreService, private platform: Platform) {
		this.initializeApp();
		...
		effect(() => {
			...
			if (
				(this.storeService.userData()[0]?.ADMIN === 1 || this.storeService.userData()[0]?.ADMIN === 3) &&
				this.appPages.find((page: any) => page.title === 'Errores') === undefined
			) {
			...
			if (
				...
				this.appPages.find((page: any) => page.title === 'Listado General') === undefined
			) {
			...
			if (
				...
				const area: string = `/area/${
					storeService.userData()[0]?.ADMIN === 1 || storeService.userData()[0]?.ADMIN === 3
						? 'all'
			...
			if (
				...
				this.appPages.find((page: any) => page.title === 'Reportes') === undefined
			) {
