...
import { Summary } from '../folder/panel/panel.page';
...
export class CrudService {
	...
	// base_url: string = 'http://192.168.1.63:3600';
	...
	exportReport() {
		return this.http.post<Blob>(`${this.base_url}/export-report`, null, {
			observe: 'response',
	...
	exportTransactions() {
		return this.http.post<Blob>(`${this.base_url}/export-transactions`, null, {
			observe: 'response',
			responseType: 'blob' as 'json',
		});
	}
	...
	getSummary(threshold: number) {
		return this.http.get<Summary>(`${this.base_url}/kpi`, {
			params: { threshold },
		});
	}
}
