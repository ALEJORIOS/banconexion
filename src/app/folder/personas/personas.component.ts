...
export class PersonasComponent implements OnInit {
	...
	async confirm() {
		...
		if (await this.checkErrors()) {
			...
			const requestBody: any = {
				...
				sex: this.newUser.controls.sex.value ? +this.newUser.controls.sex.value : null,
				phone: `${this.newUser.controls.phone.value}`,
				email: this.newUser.controls.email.value ? `${this.newUser.controls.email.value}` : null,
	...
	confirmEdition() {
		...
		if (this.checkEditionErrors()) {
			...
			const requestBody: any = {
				...
				sex: this.editUser.controls.sex.value ? +this.editUser.controls.sex.value : null,
				...
				email: this.editUser.controls.email.value ? `${this.editUser.controls.email.value}` : null,
	...
	cleanEditModal() {
		...
		this.editUser.setValue({
			...
			sex: this.currentCampist.SEX,
			phone: this.currentCampist.PHONE,
			email: this.currentCampist.EMAIL,
	...
	async checkErrors(): Promise<boolean> {
		...
		if (this.newUser.controls.sex.invalid) {
			this.alertMessage = 'El campo sexo es obligatorio';
			this.errorToast.present();
			return false;
		}
	...
	checkEditionErrors(): boolean {
		...
		if (this.editUser.controls.sex.invalid) {
			this.alertMessage = 'El campo sexo es obligatorio';
			this.errorToast.present();
			return false;
		}
